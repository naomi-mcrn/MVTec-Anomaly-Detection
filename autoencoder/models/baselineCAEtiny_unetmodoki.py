...
def build_model(color_mode):
    ...
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=0.1)(x)
    conv2 = Dropout(DROP_RATIO)(x, training=True)
    x = MaxPooling2D((2, 2), padding="same")(x)
    # ---------------------------------------------------------------------------------
    ...
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=0.1)(x)
    conv3 = Dropout(DROP_RATIO)(x, training=True)
    x = MaxPooling2D((2, 2), padding="same")(x)
    ...
    x = Dense(20 * 20 * 64, kernel_regularizer=regularizers.l2(1e-6))(x)
    x = LeakyReLU(alpha=0.1)(x)

    # x = Reshape((4, 4, encoding_dim // 16))(x)
    x = Reshape((20, 20, 64))(x)
